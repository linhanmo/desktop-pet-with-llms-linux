...
def get_sources():
    sources = glob.glob("csrc/*.cpp") + glob.glob("csrc/*.cu")
    return sources


def _get_ext_name():
    name = os.environ.get("LLM_WSL_EXTENSION_NAME", "").strip()
    return name if name else "llm_wsl_extension"
...
setup(
    ...
    version="0.1.0",
    author="Captain-Mo",
    description="Custom CUDA kernels and C++ thread management for LlamaLite WSL",
    ext_modules=[
        CUDAExtension(
            name=_get_ext_name(),
            sources=get_sources(),
