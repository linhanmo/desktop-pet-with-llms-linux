...
Ubuntu22.04 工程将若干高频算子实现为 PyTorch C++ Extension，以便在训练中减少中间张量、降低内存带宽压力，并降低 Python 调度开销。扩展按训练侧重点拆为两个模块，避免所有算子强耦合在同一个动态库中：
...
- `xentropy_forward(logits, labels, smoothing, z_loss, ignore_index) -> (losses, logsumexp)`：fused softmax-cross-entropy 前向。输入 logits 形状为 `[N, V]`、labels 为 `[N]`（int64）。输出 `losses/logsumexp` 为 float32 的 `[N]`，其中对 `ignore_index` 或非法 label 的位置写 0 并在反向中将对应梯度置零。
- `xentropy_backward(grad_losses, logits, logsumexp, labels, smoothing, z_loss, ignore_index) -> grad_logits`：前向/反向配套的 fused 反传。利用保存的 `logsumexp` 复用 softmax 分母，从而避免在 Python 侧额外 materialize softmax 概率矩阵，并支持 label smoothing 与 z-loss。
